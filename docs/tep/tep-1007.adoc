...
See xref:../auditing.adoc[Accounting Auditing and Asssurance] document for general description of this feature,
...
See xref:../journal.adoc[Journal Format] and description of Transaction metadata.
...
then xref:../git-storage.adoc[Tackler's git storage backend] could be used for that.  In that case,
...
See xref:../usage.doc[Usage Guide] and xref:../git-storage.doc[Git Storage Guide] for further info.
...
See xref:../journal.adoc[Journal Format] for further info.
...
** [x] Hard error if UUID is missing  during txn set checksum calculation +
       This is either internal error or logic error within usage of xref:../server-api.adoc[Server API]
