...
** [ ] RegisterReport
*** [ ] New metadata item, report time zone
*** [ ] Timestamps on register report rows will not have time zone info, and they will be displayed on reportTZ

====== API

* [ ] API TxnTS
** [ ] deprecate isoXXX methods
** [ ] new isoOffsetXXX methods (e.g. )
** [ ] new localXXX methods (without offsets)
...
* [ ] If reportTZ is not activated, no changes
* [ ] When reportTZ is set:
** [ ] Balance group names (e.g. truncates ts) will not have time zone info
** [ ] Balance group names hey will be displayed on reportTZ
...
* [ ] Change: separate zone name or offset from date
